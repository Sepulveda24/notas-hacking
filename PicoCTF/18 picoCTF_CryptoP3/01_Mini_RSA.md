Descripción
What happens if you have a small exponent? There is a twist though, we padded the plaintext so that (M ** e) is just barely larger than N. Let's decrypt this: [ciphertext](https://mercury.picoctf.net/static/a24cf907007a19dbf30310acad0df9e5/ciphertext)




Hints:
1.⁠ RSA [tutorial](https://en.wikipedia.org/wiki/RSA_\(cryptosystem\))
2.How could having too small of an `e` affect the security of this key?
3.Make sure you don't lose precision, the numbers are pretty big (besides the `e` value)
4.You shouldn't have to make _too_many guesses
5`pico` is in the flag, but not at the beginning

Solución 1

Con terminal linux

┌──(parallels㉿kali-linux-2022-2)-[~]
└─$ ls
 binwalk-env            docProps    fixme1.tar.gz   fixme3                   Music      ppt      shell.php   venv
'[Content_Types].xml'   Documents   fixme2          fixme3.tar.gz            myenv      Public   Templates   Videos
 Desktop                Downloads   fixme2.tar.gz  'Forensics is fun.pptm'   Pictures   _rels   '~.venv'
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~]
└─$ cd Desktop  
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop]
└─$ ls
 firefox-esr  'Parallels Shared Folders'   Retos
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop]
└─$ de re     
zsh: command not found: de
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop]
└─$ cd Retos   
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos]
└─$ ls
Actividad_16_cripto1  capture.pcap    crypto2    Forensic4  picopico.key    TareaForensic3     vulture.jpg
binwalk               capture.pcap.1  dolls.jpg  myenv      picopico.key.1  tunn3l_v1s10n.bmp
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos]
└─$ mkdir crypto3 
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos]
└─$ cd crypto3
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3]
└─$ ls
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3]
└─$ mkdir miniRSA
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3]
└─$ cd miniRSA 
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ wget https://mercury.picoctf.net/static/a24cf907007a19dbf30310acad0df9e5/ciphertext            
--2025-05-23 11:51:00--  https://mercury.picoctf.net/static/a24cf907007a19dbf30310acad0df9e5/ciphertext
Resolving mercury.picoctf.net (mercury.picoctf.net)... 18.189.209.142
Connecting to mercury.picoctf.net (mercury.picoctf.net)|18.189.209.142|:443... connected.
HTTP request sent, awaiting response... 200 OK
Length: 2039 (2.0K) [application/octet-stream]
Saving to: ‘ciphertext’

ciphertext                     100%[===================================================>]   1.99K  --.-KB/s    in 0.002s  

2025-05-23 11:51:01 (826 KB/s) - ‘ciphertext’ saved [2039/2039]

                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ ls
ciphertext
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ cat ciphertext 
N: 
e: 3

ciphertext (c): 
                                                                                                                           
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ python exp.py
                                                                                                                            
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ python exp.py
[+] Encontre raiz cubica con k=3533
Traceback (most recent call last):
  File "/home/parallels/Desktop/Retos/crypto3/miniRSA/exp.py", line 13, in <module>
    print("Texto plano como numero: " + m_root)
          ~~~~~~~~~~~~~~~~~~~~~~~~~~~~^~~~~~~~
TypeError: can only concatenate str (not "gmpy2.mpz") to str
                                                                                                                            
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ python exp.py
[+] Encontre raiz cubica con k=3533
Texto plano como numero:  
Texto desencriptaado como ascii b'                                                                                                        picoCTF{e_sh0u1d_b3_lArg3r_0b39bbb1}'
                                                                                                                            
┌──(parallels㉿kali-linux-2022-2)-[~/Desktop/Retos/crypto3/miniRSA]
└─$ 




from gmpy2 import iroot

N=1615765684321463054078226051959887884233678317734892901740763321135213636796075462401950274602405095138589898087428337758>
e=3
C=1220012318588871886132524757898884422174534558055593713309088304910273991073554732659977133980685370899257850121970812405>
 

for k in range(1,1000000):                
        candidate = C+k*N
        m_root, exact = iroot(candidate,e)
        if exact:
                print(f"[+] Encontre raiz cubica con k={k}")
                print("Texto plano como numero: " , m_root)
                print("Texto desencriptaado como ascii", bytes.fromhex(hex(m_root)[2:]))
                break


picoCTF{e_sh0u1d_b3_lArg3r_0b39bbb1}

Notas adicionales

--------------------


Referencias

terminal linux